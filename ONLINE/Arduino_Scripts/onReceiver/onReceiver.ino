...
const uint16_t window = 25; // Moving Average Algorithm window length
...
const uint16_t cut_error = 50;
const uint16_t buffer_cut_size = cut_error + (tone_off - tone_on) + cut_error;
...
const double willisonThreshold = 0.01; // Willison Amplitude threshold [mV]
...
double buffer_raw[buffer_raw_size] = {0};
...
void loop()
{
  ...
  if (write_flag) // extract and send
  {
    write_flag = false;

    extract();
    for (int i = 0; i < buffer_raw_size; i++) // reset buffer to all 0
    {
      buffer_raw[i] = 0.0;
    }
    startTimerValue0 = timer0.begin(timer0_callback, period0); // restart timer
...
void check(void)
{
  if (on_flag) // check if OFF threshold hit
  {
    if ()
    {
      off_flag = true;
  ...
  else // check if ON threshold hit
  {
    if ()
    {
      on_flag = true;
...
void extract(void)
{
  ...
  double *data;
  data = buffer_raw; // Point data to raw data buffer
  uint16_t buffer_size = buffer_raw_size; // Define buffer_size
    ...
    buffer_size = buffer_raw_size - window + 1; // Update buffer_size
  #endif
  ...
  {
    buffer_cut[i] = *(data + (tone_on - cut_error) + i);
  }
  ...
  double complexity = sqrt(var_two/var_one);
  ...
  #ifdef DEBUG
    Serial.println(fPkt.mav);
    Serial.println(fPkt.rms);
    Serial.println(fPkt.wl);
    Serial.println(fPkt.zc);
    Serial.println(fPkt.wa);
    Serial.println(fPkt.hj_a);
    Serial.println(fPkt.hj_m);
    Serial.println(fPkt.hj_c);
    Serial.flush();
  #endif
...
{
  if (buffer_0_count == buffer_raw_size - 1) // stop timer as soon as possible
  {
    timer0.end();
  }
  ...
  if (radio.available())
  {
    if (buffer_0_count < buffer_raw_size)
    {
      radio.read(&dPkt, sizeof(dPkt));
      buffer_raw[buffer_0_count + 1] = (double)(dPkt.adcVal*(sysVol/pow(2, adcRes)));
      buffer_0_count++;
    }
  }
  if (buffer_0_count == buffer_raw_size)
